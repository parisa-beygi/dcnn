...
def load_data(item_review, train_data,valid_data,test_data):
    """
    ...
    # user_voc_primary, y_train, y_valid, y_test, u_len, uid_train, iid_train, uid_valid, iid_valid, uid_test, iid_test, user_num
    i_text, i_len, item_num =\
        load_data_and_labels(item_review, train_data,valid_data,test_data)
    ...
    if not config['save_padded_text']:
        ...
        vocabulary_item_primary, vocabulary_inv_item_primary = build_vocab(item_voc_primary)
        # print len(vocabulary_user_primary)
        print len(vocabulary_item_primary)
        i_text2 = build_input_data(i_text,vocabulary_item_primary)

        save_text(i_text2, vocabulary_item_primary, vocabulary_inv_item_primary)
    ...
    return [i_text, i_len, vocabulary_item, vocabulary_inv_item, item_num]
...
def load_data_and_labels(item_review, train_data,valid_data,test_data):
    """
    ...
    return [i_text, i_len,item_num]
#   u_text,i_text,y_train,y_valid,y_test,u_len,i_len,uid_train,iid_train,uid_valid,iid_valid,uid_test,iid_test,user_num,item_num
...
def save_text(i_text, vocabulary_item, vocabulary_inv_item):
...
if __name__ == '__main__':
    ...
    i_text, i_len, vocabulary_item, vocabulary_inv_item, item_num = \
        load_data(FLAGS.item_review, FLAGS.train_data, FLAGS.valid_data, FLAGS.test_data)

    if config['save_padded_text']:
        print ('Saving Padded Text')
        save_text(i_text, vocabulary_item, vocabulary_inv_item)
    ...
    # para['u_text'] = u_text
    para['i_text'] = i_text
    output = open(os.path.join(TPS_DIR, '{}.ipara'.format(config['data_name'])), 'wb')
